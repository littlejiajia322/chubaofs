...
func (c *Cluster) createVol(name, owner string, mpCount, size, capacity, dpReplicaNum int, followerRead, needTicket bool) (vol *Vol, err error) {
	var (
	...
	if vol, err = c.doCreateVol(name, owner, dataPartitionSize, uint64(capacity), dpReplicaNum, followerRead, needTicket); err != nil {
		goto errHandler
	}
...
func (c *Cluster) doCreateVol(name, owner string, dpSize, capacity uint64, dpReplicaNum int, followerRead, needTicket bool) (vol *Vol, err error) {
	var id uint64
	...
	vol = newVol(id, name, owner, dpSize, capacity, uint8(dpReplicaNum), defaultReplicaNum, followerRead, needTicket)
