...
func (c *Cluster) createVol(name, owner string, mpCount, size, capacity int, followerRead, needTicket bool) (vol *Vol, err error) {
	var (
	...
	if vol, err = c.doCreateVol(name, owner, dataPartitionSize, uint64(capacity), defaultReplicaNum, followerRead, needTicket); err != nil {
		goto errHandler
	}
